  ...
  const plans = [
    {
      ...
      features: ['plans.free.features.0', 'plans.free.features.1', 'plans.free.features.2', 'plans.free.features.3'],
      cta: 'plans.free.cta',
      popular: false
    },
    {
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="w-full px-4 sm:px-6 py-8 sm:py-12">
          ...
          <div className="max-w-7xl mx-auto text-center mb-12 sm:mb-16">
            <h1 className="text-3xl sm:text-5xl font-semibold text-primary-foreground mb-3 sm:mb-4">{t('title')}</h1>
            <p className="text-base sm:text-xl text-secondary-foreground max-w-2xl mx-auto">{t('subtitle')}</p>
          </div>
            ...
            {plans.map((plan) => (
              <div
                ...
                className={`relative flex flex-col rounded-2xl border-2 p-6 sm:p-8 ${
                  plan.popular
                    ? 'border-primary-foreground bg-secondary-bg shadow-xl scale-105'
                    : 'border-border-new bg-primary-bg'
                }`}
              >
                {plan.popular && (
                  <div className="absolute -top-3 sm:-top-4 left-1/2 -translate-x-1/2 px-3 sm:px-4 py-1 bg-primary-foreground text-primary-bg rounded-full text-xs sm:text-sm font-medium uppercase">
                    {t('popular')}
                  </div>
                )}

                <div className="mb-4 sm:mb-6">
                  <h3 className="text-xl sm:text-2xl font-semibold text-primary-foreground mb-2">{t(plan.nameKey)}</h3>
                  <p className="text-sm sm:text-base text-secondary-foreground">{t(plan.descriptionKey)}</p>
                </div>
                ...
                <div className="mb-6 sm:mb-8">
                  {plan.price !== null ? (
                    ...
                    <div className="text-2xl sm:text-3xl font-bold text-primary-foreground">{t(plan.priceText!)}</div>
                  )}
                </div>
                ...
                <ul className="space-y-3 sm:space-y-4 mb-6 sm:mb-8 flex-1">
                  {plan.features.map((featureKey, idx) => (
                    <li key={idx} className="flex items-start">
                      <svg
                        className="w-5 h-5 text-primary-foreground mr-2 sm:mr-3 mt-0.5 flex-shrink-0"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
                      ...
                      <span className="text-sm sm:text-base text-primary-foreground">{t(featureKey)}</span>
                    </li>
                  ))}
                </ul>
            ...
            <div className="space-y-6">
              {[0, 1, 2, 3].map((idx) => (
                <div key={idx} className="border-b border-border-new pb-6">
                  ...
                  <p className="text-sm sm:text-base text-secondary-foreground">{t(`faq.questions.${idx}.answer`)}</p>
                </div>
              ))}
            </div>
          </div>
