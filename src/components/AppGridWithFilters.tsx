...
import { AppSearchFilter } from './AppSearchFilter';
...
interface AppGridWithFiltersProps {
  initialApps: AppWithTypes[];
  types: Type[];
}

export function AppGridWithFilters({ initialApps, types }: AppGridWithFiltersProps) {
  ...
  const fetchApps = useCallback(async (searchTerm: string, selectedTypeId: string | null) => {
    ...
    <section className="flex flex-col gap-8 md:gap-12 lg:gap-16 items-start p-4 md:p-10 lg:p-20 w-full">
      ...
      {apps.length > 0 && (
        <p className="text-sm text-neutral-600">
          {t('resultsCount', { count: apps.length })}
        </p>
      )}
